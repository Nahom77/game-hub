...
const useGenres = () => {
  const [genres, setGenres] = useState<Genre[]>([]); // State variable for our games
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiClient
      .get<FetchGenresResponse>('/genres', { signal: controller.signal }) // adding a configuration object
      .then(res => {
